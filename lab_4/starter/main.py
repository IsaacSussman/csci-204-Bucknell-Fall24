""" YOU WILL EDIT SEVERAL METHODS IN THIS FILE:
    class ResourceSim:
        nativeScheduler()
        roundRobinScheduler()
        randomizedLoadbalancingScheduler() # OPTIONAL FOR EC
    DO NOT EDIT THE REST OF THE FILE
"""
...
#import imp # imp is removed from 3.12
#MyQueue = imp.load_source('MyQueue', sys.path[0] + '/queue.py').MyQueue
try:
    MyQueue = load_source('MyQueue', sys.path[0] + '/queue.py').MyQueue
except: # to handle gradescope path finding
    MyQueue = load_source('MyQueue', sys.path[0] + '/../queue.py').MyQueue
...
class ResourceSim(threading.Thread):
  ...
  def __init__(self, numServers, gui):
    ...
    for i in range(self.numServers):
      self.servers.append(Server(i, self.gui.guiQueue, self.gui))
  ...
  def roundRobinScheduler(self):
    ...
    """
    return (self.lastServer+1)%self.numServers
   

  def randomizedLoadbalancingScheduler(self):
    ...
    return self.lastServer # replace this line with your implementation
    
  def select_next_server(self):
    """ Determine which server to select when the next request comes in.
    Returns the index of the server to select. """

    if self.sim_mode == NAIVE:
        return self.naiveScheduler()
    elif self.sim_mode == ROUND_ROBIN:
        return self.roundRobinScheduler()
    elif self.sim_mode == RANDOMIZED:
        return self.randomizedLoadbalancingScheduler()
...
class Server(threading.Thread):
  ...
  def __init__(self, idNum, guiQueue, gui):
    """ Overload the Thread's init to setup our Server. """
    self.idNum = idNum
    self.requestQueue = MyQueue(QUEUE_SIZE)
 ...
 Point(width/2 + self.plotWidth/2, self.plotBottom))
  ...
  def run(self):
    ...
    """while True:
        ...
        try:
          while True:
            cmd = self.guiQueue.top() # return item or raise Empty
...
def main():
	...
	r.sim_mode = ROUND_ROBIN
	if len(sys.argv) == 2:
		if sys.argv[1] == "1":
			r.sim_mode = ROUND_ROBIN
		elif sys.argv[1] == "2":
			r.sim_mode = RANDOMIZED
