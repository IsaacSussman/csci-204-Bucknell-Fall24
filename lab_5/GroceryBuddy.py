...
class GroceryApp:
  ...
  def __init__(self, list):
    ...
    self.hspace = 100  # horizonal offset to space column 2
    ...
    addInstr = Text(
      Point(self.offset, self.vspace * 2),
      "Adds after the selected item \nor first if nothing is selected.")
    ...
    self.addPic = Entry(
      Point(self.offset + self.hspace + self.ewidth * 5,
            self.vspace * 4 + self.eoffset), self.ewidth)
    ...
    self.addNum = Entry(
      Point(self.offset + self.hspace + self.ewidth * 5,
            self.vspace * 5 + self.eoffset), self.ewidth)
    self.addNum.setTextColor("white")
    self.addNum.draw(self.window)

    # Remove Button
    self.remove = Button(Point(self.offset, self.vspace * 8),
                         "Remove selected item")
    self.remove.draw(self.window)
    remInstr = Text(
      Point(self.offset, self.vspace * 9),
    ...
    updateInstr = Text(Point(self.offset, self.vspace * 14),
    ...
    self.updateNum = Entry(
      Point(self.offset + self.hspace + self.ewidth * 5,
            self.vspace * 16 + self.eoffset), self.ewidth)
    ...
    for row in range(self.length):
      r = Rectangle(
        Point(self.offset + self.hspace * 3, self.vspace + self.rheight * row),
        Point(self.offset + self.hspace * 3 + self.rwidth,
              self.vspace + self.rheight * row + self.rheight))
    ...
    for row in range(self.length):
      i = Image(
        Point(self.offset + self.hspace * 3 + .5 * self.rwidth,
              self.vspace + self.rheight * row + .5 * self.rheight), 90, 90)
      i.draw(self.window)
      self.images.append(i)
    self.nums = []
    for row in range(self.length):
      n = Text(
        Point(self.offset + self.hspace * 3 + self.rwidth + self.noffset,
  ...
  def updateGui(self):
    ...
    for index in range(length):
      try:
        ...
        print("peek raised an Exception on the perfectly good index:", index)
      ...
      try:
        if ".ppm" in img:
          img = img[:-4]
        self.images[index].setImage("img/" + img + ".ppm")
      except Exception:
        print("Bad image: " + img)
        self.images[index].setImage("img/default.ppm")
        ...
        if self.onImage(pt):
          ...
          if second < listSize:
            ...
            try:
              ...
              print(
                "3) insert method is buggy or may not have been implemented.")
      self.run()
